...
- Card padding: most use `20`, but InsightCard uses `14`, CorrelationCard/VoiceNoteCard use `16` _(intentional — denser cards)_
- Card title `marginBottom`: most use `12`, but WeeklyGrid uses `14`, RadialProgressShowcase uses `16` _(not changed)_
...
- Callback naming inconsistent: `onToggle`, `onChange`, `onTabChange`, `onClick`, `onClose` _(not changed — low impact)_
- Self-containment inconsistent _(not changed — would be a larger refactor)_
- Only `GlassCard` accepts a `style` prop for overrides; others don't _(not changed)_
